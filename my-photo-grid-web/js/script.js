let posts = []; let filteredPosts = []; let currentPage = 1; const postsPerPage = 20;
...
function updateProgress(progress) { document.getElementById('progressText').innerText = ${progress}%; }

async function loadAllPosts() { showLoader(); posts.length = 0; currentPage = 1;

try { const indexRes = await fetch('data/index.json', { cache: 'no-store' }); const indexData = await indexRes.json(); const lastModified = indexData.lastModified;

const db = await openDB();
const cachedVersion = localStorage.getItem('indexVersion');

let useCache = cachedVersion === lastModified;
...
if (useCache && cachedPosts && cachedPosts.length > 0) {
  posts = cachedPosts;
} else {
  let loadedCount = 0;
...
} catch (err) { console.error("Gagal load post:", err); } finally { hideLoader(); filterPosts(window.location.hash.replace('#', '') || 'beranda', false); } }
...
function openDB() { return new Promise((resolve, reject) => { const request = indexedDB.open('KeylapoiDB', 1); request.onerror = () => reject('Gagal buka database'); request.onsuccess = () => resolve(request.result); request.onupgradeneeded = function (e) { const db = e.target.result; if (!db.objectStoreNames.contains('posts')) { db.createObjectStore('posts', { keyPath: 'id' }); } }; }); }

function savePost(db, post) { return new Promise((resolve, reject) => { const tx = db.transaction('posts', 'readwrite'); const store = tx.objectStore('posts'); store.put(post); tx.oncomplete = () => resolve(); tx.onerror = () => reject('Gagal simpan post'); }); }

function getCachedPosts(db) { return new Promise((resolve, reject) => { const tx = db.transaction('posts', 'readonly'); const store = tx.objectStore('posts'); const request = store.getAll(); request.onsuccess = () => resolve(request.result); request.onerror = () => reject('Gagal ambil post dari cache'); }); }

function clearCache(db) { return new Promise((resolve, reject) => { const tx = db.transaction('posts', 'readwrite'); const store = tx.objectStore('posts'); const request = store.clear(); request.onsuccess = () => resolve(); request.onerror = () => reject('Gagal hapus cache'); }); }

function manualRefresh() { location.reload(); }

const updateWorker = new Worker('js/worker.js'); updateWorker.postMessage('start');

updateWorker.onmessage = function (e) { if (e.data === 'update') { checkForUpdates(); } };

async function checkForUpdates() { try { const response = await fetch('data/index.json', { cache: "no-store" }); const indexData = await response.json(); const newVersion = indexData.lastModified;
...
if ('serviceWorker' in navigator && 'SyncManager' in window) { navigator.serviceWorker.ready.then(swRegistration => { swRegistration.sync.register('sync-update').catch(err => console.error('Gagal register sync:', err)); }); }

window.addEventListener('hashchange', () => { filterPosts(window.location.hash.replace('#', '') || 'beranda', false); });

window.addEventListener('load', async () => { await loadAllPosts(); });
